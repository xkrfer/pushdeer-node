...
export class UpdateDeviceDto extends AuthDto {
  ...
  @ApiProperty({ description: '是否轻应用,0为轻应用，1为App', default: 0, example: 0, required: false })
  @IsOptional()
  @Matches(/^[01]$/, { message: 'is_clip值错误' })
  is_clip: 0 | 1;

  @ApiProperty({ description: '应用类型,可选值：ios/android', default: 'ios', example: 'ios', required: false })
  @IsOptional()
